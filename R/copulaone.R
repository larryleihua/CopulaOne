...
plotCopulaOne <- function(para, marg="normal", drawlabels=F, flag=1, integration=F, resolution=30, copula_family="PPPP", main=NULL)
{
  ...
  if(marg == "normal")
  {
    ...
    contour(zvec,zvec, denmat, drawlabels=drawlabels, levels = round(quantile(rangeforplot,seq(0.05, 0.95, length=10), na.rm=T),2), labcex = 1, main=main, xlim = c(-2.5,2.5), ylim = c(-2.5,2.5))
  } else if(marg == "uniform")
  {
    ...
    contour(Fvec, Fvec, denmat, drawlabels=drawlabels, levels = round(quantile(rangeforplot,seq(0.05, 0.95, length=10), na.rm=T),2), labcex = 1, main=main, xlim = c(0,1), ylim = c(0,1))
  }
  invisible(denvec)
}
